...
template<typename T, typename... S> inline void srt(T a, S... args) { sort(begin(a), end(a), args...); }
/* do not delete */
...
struct node {
	...
	node (ll _s, ll _e, ll A[] = NULL): s(_s), e(_e), mn(0), mx(0), sum(0), lset(0), add_val(0), set_val(0), l(NULL), r(NULL) {
		if (A == NULL) return;
